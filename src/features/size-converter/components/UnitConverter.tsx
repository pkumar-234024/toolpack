...
import { setFromValue, setFromUnit, setToUnit } from '../sizeSlice';
  ...
  return (
    <div className="space-y-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
        ...
        >
          <Card className="p-8 saas-card-flat border-none space-y-6">
             <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-full flex items-center gap-1"><Zap className="w-3 h-3" /> FROM</span>
             </div>
        ...
        >
          <Card className="p-8 saas-card-flat bg-slate-900 border-none space-y-6 overflow-hidden relative">
             <motion.div 
               animate={{ rotate: 360 }}
               transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
               className="absolute -top-24 -right-24 w-48 h-48 bg-emerald-500/5 rounded-full blur-3xl"
             />
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-6 pt-4">
         <Button 
           ...
           leftIcon={<Zap className="w-5 h-5 text-indigo-500" />}
         >
            Save to Records
         </Button>
